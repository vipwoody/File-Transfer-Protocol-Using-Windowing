...
int main(void){
	...
	try{        		 
...

		{
			...
			if(!(outfds=select(1,&readfds,NULL,NULL,tp))) {} //timed out, return

			int size = sizeof(sa_in);

			if (outfds == 1) {//receive frame
				if(ibytesrecv = recvfrom(s, (char*)& packet, sizeof(packet),0, (struct sockaddr*)&sa_in, &size) == SOCKET_ERROR)
					throw "Receive error in server program\n";
				cout << "yay?" << endl;
			}
			if (ibytesrecv != 0)
				cout << ibytesrecv << endl;
